...
$takimid = filter_input(INPUT_GET, "id", FILTER_VALIDATE_INT);
if (!$takimid) {
    http_response_code(404);
    exit("Team not found.");
}
$sorgu=$db->prepare("SELECT * FROM takimlar where id = $takimid");
$sorgu->execute();
            ...
            <div class="card-header p-3">
                <div class="league-content">
                    <div class="league-img">
                        <img src="<?=$satir["logo"]?>" width="70" alt="fb" title="fb">
                    </div>

                    <div class="league-txt">
                        <h1><?=$satir["isim"]?></h1>
                        <p>Stadyum: <strong>Şükrü Saraçoğlu</strong></p>
                        <p>Kapasite: <strong>30.000</strong></p>
                    </div>
            ...
            <div class="card-body pt-0">
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-sonuclar" role="tabpanel"
                        ...
                        <a href="league.html" class="divider d-flex align-items-center">
                           <!-- <img src="assets/img/<?=$satirlig["logo"]?>" alt="" width="20" class="me-1">--> <?=$satirlig["isim"]?>
                        </a>
                        <?php
                        
                        $sorguh2h = $db->prepare("SELECT * FROM maclar WHERE durum = 4 AND (takim1_id = :takim1_id OR takim2_id = :takim1_id) ORDER BY mac_tarihi DESC");
                        $sorguh2h->execute(array(':takim1_id' => $takimid));
                        while($satirh2h = $sorguh2h->fetch(PDO::FETCH_ASSOC)){ 
                        $takim1id=$satirh2h["takim1_id"];
                        $takim2id=$satirh2h["takim2_id"];
                        
                        if($takim1id == $takimid){
                            $anatakim = $satirh2h["takim1_skor"];
                            $digertakim = $satirh2h["takim2_skor"];
                        }elseif($takim2id == $takimid){
                            $anatakim = $satirh2h["takim2_skor"];
                            $digertakim = $satirh2h["takim1_skor"];
                        }
                        ...
                        ?>
                        <a href="#" class="match-list-item team">
                            <span><?= tarihDuzeltSaatli($satirh2h["mac_tarihi"]) ?></span>
                            <div class="score-badge badge-success">G</div>
                            <?php if($anatakim > $digertakim){ ?>
                                <div class="score-badge badge-success">G</div>
                                        <?php }elseif($anatakim < $digertakim){ ?>
                                            <div class="score-badge badge-danger">M</div>
                                        <?php }elseif($anatakim == $digertakim){ ?>
                                            <div class="score-badge badge-warning">B</div>
                                        <?php }else{ ?>
                                            <div class="score-badge badge-light">?</div>
                                        <?php } ?>
                            <div class="team-row">
                                <div class="team">
                                    <img src="<?= $detaysatirtakim1["logo"] ?>" alt="" width="20" height="20">
                                    <p><strong><?= $detaysatirtakim1["isim"] ?></strong></p>
                                </div>
                                <div class="result"><strong><?= $satirh2h["takim1_skor"] ?></strong></div>
                            </div>
                            <div class="team-row">
                                <div class="team">
                                ...
                                <div class="result"><?= $satirh2h["takim2_skor"] ?></div>
                            </div>
                        </a>
                        <?php } ?>
                    ...
                    <div class="tab-pane fade" id="nav-fikstur" role="tabpanel" aria-labelledby="nav-fikstur-tab"
                        tabindex="0">
                        <a href="league.html" class="divider d-flex align-items-center">
                            <!--<img src="assets/img/tr.png" alt="" width="20" class="me-1">-->  <?=$satirlig["isim"]?>
                        </a>
                        <?php
                        
                        $sorguh2h = $db->prepare("SELECT * FROM maclar WHERE durum = 0 AND (takim1_id = :takim1_id OR takim2_id = :takim1_id) ORDER BY mac_tarihi DESC");
                        $sorguh2h->execute(array(':takim1_id' => $takimid));
                        while($satirh2h = $sorguh2h->fetch(PDO::FETCH_ASSOC)){ 
                        $takim1id=$satirh2h["takim1_id"];
                        $takim2id=$satirh2h["takim2_id"];
